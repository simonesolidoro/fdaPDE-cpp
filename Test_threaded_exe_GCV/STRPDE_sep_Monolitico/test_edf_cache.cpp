...
int main(int argc, char** argv){
    using edf_cache_t = std::unordered_map<
          std::array<double, 2>, double, internals::std_array_hash<double, 2>>;
    edf_cache_t edf_cache;
    if(argc < 2){std::cerr<<"in input: size_grid \n";}
    ...
    for (int run = 0; run <2; run++){
        ...
        auto model_gcv = m.gcv_par(edf_cache, 100, 476813); //crea passando edf cache
        auto start = std::chrono::high_resolution_clock::now();
        optimizer.optimize(fdapde::execution_par, model_gcv, lambda_grid, granularity);
