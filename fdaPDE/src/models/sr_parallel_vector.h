...
template <class VariationalSolver, int n_worker = 1>
...
class SRPDE {
   ...
   public:
    ...
    // fitting
    template <typename... Args> auto fit(int worker_id = 0, Args&&... args) { return solver_.fit(worker_id, std::forward<Args>(args)...); }
    // observers
    ...
    struct gcv_t : public ScalarFieldBase<n_lambda, gcv_t> {
        ...
        gcv_t(SRPDE* model) : gcv_t(model, std::vector<edf_cache_t>(n_worker)) { }
        gcv_t(SRPDE* model, int r, int seed) : gcv_t(model, std::vector<edf_cache_t>(n_worker), r, seed) { }
       ...
       private:
        SRPDE* model_;
        int n_ = 0, q_ = 0;
        std::vector<edf_cache_t> edf_cache_; //per ora vector, poi meglio globale e accesso sicuro tramite shared-mutex  
...
// deduction guide per il momemnro tolta
// template <typename GeoFrame, typename Penalty>
// SRPDE(const std::string& formula, const GeoFrame& gf, Penalty&&)
//   -> SRPDE<typename std::decay_t<Penalty>::template solver_t<1>, 1>; // per n_worker>1 va specificato nel main 
