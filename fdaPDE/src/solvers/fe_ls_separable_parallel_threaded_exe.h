   ...
   public:
	...
	lambda_saved_[worker_id] = lambda;
        ...
        if (n_covs_ == 0) { 
            ...
            x = woodbury_system_solve(invA_[worker_id], U_, XtWX_, V_, b_[worker_id]);
            f_[worker_id] = x.topRows(n_dofs_);
    ...
    double ftPf(double lambda_D, double lambda_T, int worker_id = 0) {
        if (std::array<double, n_lambda> {lambda_D, lambda_T} != lambda_saved_[worker_id] || W_changed_[worker_id]) {
            fit(lambda_D, lambda_T, worker_id);
        }
        return f_[worker_id].dot(P(lambda_D, lambda_T) * f_[worker_id]);
    }
    template <typename LambdaT>
        requires(internals::is_vector_like_v<LambdaT>)
    double ftPf(const LambdaT& lambda, int worker_id) {
        fdapde_assert(lambda.size() == n_lambda);
        return internals::apply_index_pack<n_lambda>([&]<int... Ns>() { return ftPf(lambda[Ns]...,worker_id); });
    }
